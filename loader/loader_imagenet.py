...
class ImageNetDataset(ImageFolder):
    ...
    def __getitem__(self, index):
        img, target = super(ImageNetDataset, self).__getitem__(index)
...
class ImageNetLoader(BaseLoader):
    def __init__(self,
                 root: str='/grand/QTensor/',
                 filename: str='ImageNet',
                 random_state: int=42,
                 download: int=0):
        ...
        if download:
            ...
            download_script = './scripts/data/imagenet.sh'
        ...
        def get_dataset():
            print(self.path)
            train_set = ImageNetDataset(Path(self.path) / 'train',
                                            transform_train)

            test_set = ImageNetDataset(Path(self.path) / 'val',
                                           transform_test)
